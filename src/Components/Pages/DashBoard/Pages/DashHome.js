...
const DashHome = () => {
  ...
  useEffect(() => {
    fetch(
      `https://gentle-fortress-64006.herokuapp.com/bookings?email=${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  const handleDelUser = (id) => {
    const proceed = window.confirm("Are you sure,you wanna delete this item?");
    if (proceed) {
  ...
  return (
    <div className="text-center container my-3 bg-white">
      <h1>My Orders</h1>
      <h3 className="mb-3">Total Orders: {orderData.length}</h3>
      <div className="row">
        {isLoading ? (
          orderData?.map((od) => (
            <>
      ...
      <Link to="/cycles" className="btn btn-info mt-3">
        Explore More Cycles
      </Link>
    </div>
  );
